...
export default function PrintOverlay({ venues, events, categoryColors, config = CONFIG, banners = [] }: PrintOverlayProps & { config?: typeof CONFIG }) {
    const { START_HOUR, END_HOUR, HOUR_HEIGHT, GRID_INTERVAL } = config;
    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* 右側：メイングリッド */}
                <EventGrid venues={venues} events={events} categoryColors={categoryColors} config={config} banners={banners} />
            </div>
        </div>
    );
}
