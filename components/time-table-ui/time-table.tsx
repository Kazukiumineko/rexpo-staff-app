...
import { venues, events, CATEGORY_COLORS, CONFIG as CONFIG_MAIN, banners as bannersMain } from "./data";
import { venuesSub, eventsSub, CATEGORY_COLORS_SUB, CONFIG as CONFIG_SUB, bannersSub } from "./data-sub";
...
export default function TimeTable() {
    ...
    const currentVenues = tab === 'main' ? venues : venuesSub;
    const currentEvents = tab === 'main' ? events : eventsSub;
    const currentColors = tab === 'main' ? CATEGORY_COLORS : CATEGORY_COLORS_SUB;
    const currentConfig = tab === 'main' ? CONFIG_MAIN : CONFIG_SUB;
    const currentBanners = tab === 'main' ? bannersMain : bannersSub;
    ...
    // ベースサイズ定義
    const BASE_SIZES = {
        colWidth: currentConfig.COLUMN_WIDTH,
        hourHeight: currentConfig.HOUR_HEIGHT,
        headerHeight: currentConfig.HEADER_HEIGHT,
        timeColWidth: currentConfig.TIME_COL_WIDTH,
        minCardHeight: 40 // これはCONFIGにないので固定
    };
        ...
        <>
            ...
            <section
                ...
                style={{
                    ...
                    "--fs-time-col": `calc(15px * ${zoomScale})`,
            ...
            >
                ...
                <div className="max-w-[1600px] mx-auto relative min-h-[80vh]">
                    ...
                    <div className="sticky top-0 z-[95] bg-[#f1f1f1] w-full print:hidden pb-1 pt-14 lg:pt-[72px]">
                        <div className="flex flex-col md:flex-row items-start md:items-center gap-4 mb-4 px-4 md:px-0 md:ml-14">
                            {/* ▼ ズームバー (スマホのみ -> PCも表示) */}
                            ...
                            <div className="flex bg-white rounded-full p-1 shadow-sm border border-gray-200">
                                <button
                                    onClick={() => setTab('main')}
                                    className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all duration-300 ${tab === 'main'
                                        ? 'bg-[#092040] text-white shadow-md'
                                ...
                                <button
                                    onClick={() => setTab('sub')}
                                    className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all duration-300 ${tab === 'sub'
                                        ? 'bg-[#092040] text-white shadow-md'
                                        : 'text-gray-500 hover:bg-gray-100'
                                        }`}
                                >
                        ...
                        {/* ヘッダーエリア (Web表示用) */}
                        <TableHeader scrollRef={headerScrollRef} venues={currentVenues} />
                    </div>
                    ...
                    <div className="relative">
                        ...
                        >
                            {/* コンテンツラッパー (全幅) */}
                            <PrintOverlay venues={currentVenues} events={currentEvents} categoryColors={currentColors} config={currentConfig} banners={currentBanners} />
                        </div>
                    </div>
